...
def clip_rm_inf_and_change_inout_type_int32(src_onnx, dst_onnx, batch=1):
    graph = gs.import_onnx(onnx.load(src_onnx))
    # change_inout_type(graph)
    for node in graph.nodes:
        if node.name == "/transformer/text_model/Trilu":
            fp32_inf = gs.Constant("fp32_inf", np.ones((batch, 77, 77), dtype= np.float32) * -10000)
            node.inputs[0] = fp32_inf
