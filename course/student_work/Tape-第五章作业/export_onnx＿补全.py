...
def export_control_net_model():
    ...
    # 噪声的潜在表示，形状为 (1, 4, 32, 48)
    x_noisy = torch.randn(1, 4, 32, 48, dtype=torch.float32)
    # 条件输入（如边缘图、深度图等），形状为 (1, 3, 256, 384)
    ...
    # 导出 ControlNet 模型为 ONNX 格式，参数一是模型，参数二是输入张量，参数三是输出文件名，参数四是否打印信息，参数五是ONNX 的操作集版本，参数六是启用常量折叠优化，减少模型复杂度
    # 参数七是输入和输出的名称，参数八是初始权重可作为输入节点允许替换不同的controlenet
    torch.onnx.export(
        control_net,
...
def export_controlled_unet_model():
    ...
    # ControlNet 的 13 个中间层输出也作为DDPM 的中间输入，实现对UNet各层级的细粒度控制
    control_list = [
...
def export_decoder_model():
    ...
    # keep_initializers=True 使参数可见，便于逐层量化
    torch.onnx.export(
        decode_model.cpu(),
        (latent),
        onnx_path,
