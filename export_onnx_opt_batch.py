...
def export_clip_model():
    ...
    onnx_path = "./onnx/CLIP_batch.onnx"
    opt_onnx_path = "./onnx/CLIP_opt_batch.onnx"

    tokens = torch.zeros(2, 77, dtype=torch.int32)
    ...
    surgeon_graph.clip_rm_inf_and_change_inout_type(onnx_path, opt_onnx_path, batch=2)
...
def export_control_net_model():
    ...
    onnx_path = "./onnx/ControlNet_batch.onnx"
    opt_onnx_path = "./onnx/ControlNet_opt_batch.onnx"
    ...
    surgeon_graph.add_plugins_and_change_inout_type(onnx_path, opt_onnx_path, True)
...
def export_controlled_unet_model():
    ...
    control_list = [
        torch.randn(2, 320, 32, 48, dtype=torch.float32),
        torch.randn(2, 320, 32, 48, dtype=torch.float32),
        torch.randn(2, 320, 32, 48, dtype=torch.float32),
        torch.randn(2, 320, 16, 24, dtype=torch.float32),
        torch.randn(2, 640, 16, 24, dtype=torch.float32),
        torch.randn(2, 640, 16, 24, dtype=torch.float32),
        torch.randn(2, 640, 8, 12, dtype=torch.float32),
        torch.randn(2, 1280, 8, 12, dtype=torch.float32),
        torch.randn(2, 1280, 8, 12, dtype=torch.float32),
        torch.randn(2, 1280, 4, 6, dtype=torch.float32),
        torch.randn(2, 1280, 4, 6, dtype=torch.float32),
        torch.randn(2, 1280, 4, 6, dtype=torch.float32),
        torch.randn(2, 1280, 4, 6, dtype=torch.float32),
    ]
    ...
    onnx_path = "./onnx/ControlledUnet_batch"
    os.makedirs(onnx_path, exist_ok=True)
    onnx_path = onnx_path + "/ControlledUnet_batch.onnx"
    opt_onnx_path = "./onnx/ControlledUnet_opt_batch"
    os.makedirs(opt_onnx_path, exist_ok=True)
    opt_onnx_path = opt_onnx_path + "/ControlledUnet_opt_batch.onnx"
